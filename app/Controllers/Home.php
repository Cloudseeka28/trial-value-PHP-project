...
class Home extends BaseController
{
    ...
    public function save(){
     ...
     $Compensation_method =  implode(',', $Compensation_method);
        $form_data = [
    ...
    public function cal($data,$insertId){
        ...
        $Compensation_methodArray =  explode (",", $data['Compensation_method']); 
        ...
        if(in_array('Lost income',$Compensation_methodArray))
      {  
        $Lostincome  = ($data['Number_of_visits_per_study']*($data['Average_time_spent_in_clinic_per_visit']+$data['Average_travel_time_per_clinic_visit_round_trip']) * $data['Minimum_and_Mean_Hourly_Earning_PPP_ILO_USD']);
       ...
       if(in_array('Participation stipend',$Compensation_methodArray))
      { 
        ...
        $Travelaccomodationexpense =  0;

       if(in_array('Reimbursement',$Compensation_methodArray))
      { 
        ...
        $FinancialImapact = $EstimatedReimbursementTotal +$EstimatedCompensationTotal;
        // var_dump($FinancialImapact);
        
        return array(
            "Lostincome"=>$this->cheackNumber($Lostincome),
            "Stipend"=>$this->cheackNumber($Stipend),
            "EstimatedCompensationTotal"=>$this->cheackNumber($EstimatedCompensationTotal),
            "Travelaccomodationexpense"=>$this->cheackNumber($Travelaccomodationexpense),
            "Patientsupportexpense"=>$this->cheackNumber($Patientsupportexpense),
            "EstimatedReimbursementTotal"=>$this->cheackNumber($EstimatedReimbursementTotal),
            "BurdenImpact"=>$this->cheackNumber($BurdenImpact),
            "BurdenImpactRisk"=>$BurdenImpactRisk,
            "id"=>$insertId,
            "FinancialImapact" => $this->cheackNumber($FinancialImapact)
    );

    }

    public function cheackNumber($number){
        $value = '';

            if ($number >= 0){
                $value = "$".number_format($number,2);
            }else{
                $value =  "-$".  str_replace("-","",''.number_format($number,2).'');
            }
            return $value;
    }
    ...
    {
        // Load the model and fetch form data by ID
        $formModel = new ValuePatientFinancialBurdenModel();
        ...
        // Load the HTML view and pass data to it
        ...
        $dompdf->stream("burden_calculator_$id.pdf", ["Attachment" => true]);
    }
